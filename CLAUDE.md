...
backend/    Spring Boot modular monolith
  ...
  resume/     PDF validation, extraction, normalization        (phase 2)
  jobposting/ safe fetching, extraction strategies             (phases 3-4)
  document/   shared normalization, provenance, PII redaction  (phase 2+)
...
- Constructor injection only. `ArchitectureRulesTest` fails the build on field injection, on console
  output, and on the domain depending on the API layer.
